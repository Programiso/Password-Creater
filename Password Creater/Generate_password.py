...
class GeneratePassword():
    ...
    Opcjonalne parametry: ptype=1, min_lenth=5, max_lenth=30, password_color='green', language='English', int_error_msg=[...]"""
    
    def __init__(self, letters, numbers, symbols, ptype=1, min_lenth=5, max_lenth=30, password_color="green", language="English", int_error_msg="Please enter a numerical value", e_priority="[{1} 2 3]"):
    ...
    def chceck_values(self):
        ...
        welcome_steps = (
            "\n\n\n\n\n\n\n\n\n\n\nCreate a password with assistant------------------Steps 1 [2] 3]", #0
            "\n\n\n\n\nPassword type: ", #1
            "Minimum lenth od password: ", #2
            "Maximum lenth of password: ", #3
            "\n\nAre theese values right?(Y/N): " #4
        )#krotka z dalszymi instrukcjami
        for i in range(0,4): #indexuje liste
            if i == 0 or i == 4:
                text = ""
            elif i == 1:
                if self.priority == 1:
                    ...
                    text = '[1 2 3 {overflow!}]' #dodaje [1 2 3 4 5] i zaznacza wybor przy priority
            elif i == 2:
                text = str(self.min_lenth) #przeksztalcza int ---> str
            elif i == 3:
                text = str(self.max_lenth) #przeksztalcza int ---> str 

            print(welcome_steps[i] + text) #printuje to wszystko
            text = "" #resetuje tekst
    ...
    def break_numeral_password(self):
        pass

    def jakas_inna_funkcja(self):
        is_rotated = [False] #lista, ktora sprawdza czy wszystkie "rotacje" zostaly wykonane(false = nie zrobilo), jesli wszystkie rotacje sa True, wtedy tworzy nowe "kolo"
        last_rotation = 0 #zmienna, ktora rejestruje ostatnia rotacje
        first_rotation = 0 #zmienna, ktora rejestruje poczatek
        password = []
        assebled_password = ""
        while assebled_password != user_password:    
            for i in letters:
                #print(i) #Debuguje jaka wartosc ma i
                if i == "M":
                    if last_rotation == len(is_rotated) - 1:
                        is_rotated.pop()
                        is_rotated.append(True)
                        last_rotation += 1
                    if False in is_rotated:
                        pass
                    else:
                        password.append(i)
 
                password.append(i) #wklada litere do listy
                assebled_password = ""
                for j in password:
                    assebled_password += j
                if assebled_password == user_password:
                    end = datetime.now().time() 
                    print(f"It took {end}")
                    break
                print(assebled_password)
                print(password)
                print(is_rotated)
